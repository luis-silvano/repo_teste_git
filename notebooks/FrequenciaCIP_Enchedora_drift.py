...
df_calendario_assepsias = df_assepsias_previsao.withColumn('NUM_LINHA', row_number().over(wd_partition)).\
  withColumn(
    'DESC_ETAPA', 
    # FIX: Distinguir Assepsia Diária (periodicidade) das Assepsias por parada
    # Assepsia Diária não deve exigir parada correspondente no INNER JOIN
    when(col('NUM_LINHA') == 1, lit('Previsto e Realizado (Assepsia Diária)'))
    .otherwise(
        concat(
...
# BUGFIX: Validar que assepsias realizadas têm parada correspondente
# Problema: Assepsias cobradas sem parada correspondente (bug reportado cliente F.A.Claras L511)
# 
# IMPORTANTE: 
# - Assepsias 'Previsto e Realizado' (por parada) → DEVEM ter parada ≥40min correspondente
# - Assepsias 'Previsto e Realizado (Assepsia Diária)' (por periodicidade) → NÃO exigem parada
#
# O filtro abaixo captura apenas as assepsias que dependem de parada (sem o marcador 'Assepsia Diária')
df_previsao_assepsia_com_parada = (
    df_previsao_assepsia
    .filter(col('DESC_ETAPA') == 'Previsto e Realizado')  # Apenas assepsias por parada (sem 'Assepsia Diária')
    .alias('assepsia_realizada')
    .join(
        df_paradas_trechos_assepsia.alias('paradas'),
        on=(
            (col('assepsia_realizada.SK_LINHA_PRODUCAO') == col('paradas.SK_LINHA_PRODUCAO')) &
            (col('assepsia_realizada.NUM_VALOR') >= col('paradas.DATA_REF_INICIO_TRECHO') - expr('INTERVAL 6 HOUR')) &
            (col('assepsia_realizada.NUM_VALOR') <= col('paradas.NUM_VALOR') + expr('INTERVAL 1 HOUR'))
        ),
        how='inner'
    )
    .select('assepsia_realizada.*')
    .drop_duplicates()
)

# Combina:
# 1. Assepsias por parada que têm parada correspondente (validadas)
# 2. Assepsias Diárias por periodicidade (não precisam de validação com parada)
# 3. Previstos não realizados
df_previsao_assepsia = (
    df_previsao_assepsia_com_parada
    .unionByName(
        df_previsao_assepsia.filter(col('DESC_ETAPA') != 'Previsto e Realizado'),
...
df_trocas_sem_assepsia = (
    ...
    .join(
        df_previsao_assepsia
        # FIX: Incluir tanto assepsias por parada quanto Assepsias Diárias na verificação
        .filter(col('DESC_ETAPA').contains('Previsto e Realizado'))
        .alias('previsao'),
        on=[
            col('trocas.SK_LINHA_PRODUCAO') == col('previsao.SK_LINHA_PRODUCAO'),
