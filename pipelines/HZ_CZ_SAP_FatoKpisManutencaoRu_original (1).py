...
df_iw37n_ecc = df_iw37n_hz.join(df_unidades, ['Centro_planejamento'], how='left')
df_iw37n_hana = df_ordens_manutencao_hz_hana_final.join(df_unidades, df_ordens_manutencao_hz_hana_final['Centro_planejamento'] == df_unidades['SAP_CODIGO_HANA'], how='left').drop(df_unidades.Centro_planejamento)
...
df_iw37n_ecc = df_iw37n_ecc.join(df_rollouthana, df_iw37n_ecc['COD_UNIDADE_LAKE'] == df_rollouthana['SK_UNIDADE'], how='left').select(df_iw37n_ecc['*'], df_rollouthana['DATA_ROLLOUT'])
...
# Aplica o to_date em todas as colunas de data em um único encadeamento
for c in colunas_de_data:
    df_iw37n = df_iw37n.withColumn(c, to_date(col(c), data_format))

# Aplica o regexp_replace em todas as colunas de hora em um único encadeamento
for c in colunas_de_horas:
    df_iw37n = df_iw37n.withColumn(c, regexp_replace(col(c), string_regex_pattern, string_regex_replacement))
...
# DBTITLE 1,Cria novo DF de trabalho para iniciar tratamento de calculo de trabalho
df_iw37n_trabalho = df_iw37n_final.withColumn('Data_fim_real_formatada',date_format(to_date(col('Data_fim_real'),'yyyyMMdd'),'yyyyMM'))\
                                  .select('Ordem','Operacao','Sub_Operacao','Trabalho_real','Data_fim_real_formatada').distinct()

# COMMAND ----------

# DBTITLE 1,DF de trabalho recebe a função de rank para ordenar periodos da mesma Ordem/Operacao
w = Window.partitionBy('Ordem','Operacao','Sub_Operacao').orderBy('Data_fim_real_formatada')

df_iw37n_trabalho = df_iw37n_trabalho.withColumn("drank", dense_rank().over(w))\
                                     .withColumn('Sub_Operacao', coalesce(col('Sub_operacao'),lit(0)))
                                       

df_iw37n_aux = df_iw37n_trabalho.withColumn('drank',col('drank') + 1)\
                                .withColumn('Sub_Operacao', coalesce(col('Sub_operacao'),lit(0)))\
                                .withColumnRenamed('Trabalho_real', 'Trabalho_real_anterior')
 


# COMMAND ----------

# DBTITLE 1,Junção das DFs de trabalho e auxiliar para juntar dados de periodos diferentes
df_iw37n_trabalho = df_iw37n_trabalho.join(df_iw37n_aux,(df_iw37n_trabalho['Ordem'] == df_iw37n_aux['Ordem']) &
                                                       (df_iw37n_trabalho['Operacao'] == df_iw37n_aux['Operacao']) &
                                                       (df_iw37n_trabalho['Sub_Operacao'] == df_iw37n_aux['Sub_Operacao']) &
                                                       (df_iw37n_trabalho['drank'] == df_iw37n_aux['drank']),'left')\
                                                       .select(df_iw37n_trabalho['*'],df_iw37n_aux['Trabalho_real_anterior'])\
                                                       .withColumn('Sub_operacao', when(col('Sub_operacao') == 0, lit(None)).otherwise(col('Sub_operacao')))\
                                                       .withColumn('Trabalho_real_calculado', col('Trabalho_real') - coalesce(col('Trabalho_real_anterior'),lit(0)))

# COMMAND ----------

# DBTITLE 1,É criado um df final que recebe o DF final original e junta com os novos dados de trabalho
df_iw37n_final_aux = df_iw37n_final.join(df_iw37n_trabalho, (df_iw37n_final['Ordem'] == df_iw37n_trabalho['Ordem']) &
                                                          (df_iw37n_final['Operacao'] == df_iw37n_trabalho['Operacao']) &
                                                          (coalesce(df_iw37n_final['Sub_Operacao'],lit(0)) == coalesce(df_iw37n_trabalho['Sub_Operacao'],lit(0))) &
                                                          (df_iw37n_final['ANO_MES_RU'] == df_iw37n_trabalho['Data_fim_real_formatada']),'left')\
                                                          .select(df_iw37n_final['*'], df_iw37n_trabalho['Trabalho_real_calculado'] )
...
df_iw37n_final_aux = df_iw37n_final_aux.repartition(100)
